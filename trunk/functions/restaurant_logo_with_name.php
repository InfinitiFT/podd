    ...
    if (isset($_POST["submit"])) {
        ...
        if (!empty($_FILES["image"]['tmp_name'])) {
            if ((($_FILES["image"]["type"] == "image/gif") || ($_FILES["image"]["type"] == "image/jpeg") || ($_FILES["image"]["type"] == "image/pjpeg") || ($_FILES["image"]["type"] == "image/png") || ($_FILES["image"]["type"] == "image/jpg"))) {
                ...
                $target_path    = $_SERVER['DOCUMENT_ROOT'] . "/PROJECTS/IOSNativeAppDevelopment/trunk/uploads/home_page_image/";
                $target_path    = $target_path . $home_page_image1;
                if (move_uploaded_file($_FILES['image']['tmp_name'], $target_path)) {
                    if (mysqli_query($GLOBALS['conn'], "UPDATE `home_page_image` SET `image_message`= '". $image_message . "',`image_url`= '". 'uploads/home_page_image/'.$home_page_image1 . "' WHERE `image_id` = '" . decrypt_var($_GET['id']) . "'")) {
		...
		else if (!empty($image_message))
		{
			if (mysqli_query($GLOBALS['conn'], "UPDATE `home_page_image` SET `image_message`= '". $image_message . "' WHERE `image_id` = '" . decrypt_var($_GET['id']) . "'")) {
                        $_SESSION["successmsg"] = "Home page edited successfully.";
                        header('Location:home_page_listing.php');
                    } else {
                        $_SESSION["errormsg"] = "insertion error.";
