 <?php
  ...
  $result=array();
	...
	if(!empty($meal)){
		$data = mysqli_query($GLOBALS['conn'],"SELECT d.*,( 3959 * acos( cos( radians($lat) ) * cos(radians(latitude ) ) * cos( radians( longitude ) - radians($long) ) + sin( radians($lat) ) * sin( radians(latitude ) ) ) ) AS distance FROM `restaurant_details` as d join restaurant_meal_details as m on m.restaurant_id = d.restaurant_id WHERE m.meal =".$meal." AND m.deliver_food = ".$deliver_food." GROUP BY d.restaurant_id");
		
	}else if(!empty($location)){
		$data = mysqli_query($GLOBALS['conn'],"SELECT d.*,( 3959 * acos(cos(radians($latt) ) * cos( radians(latitude ) ) * cos(radians(longitude ) - radians($longg) ) + sin( radians($latt) ) * sin( radians(latitude ) ) ) ) AS distance  FROM `restaurant_details` as d join restaurant_meal_details as m on m.restaurant_id = d.restaurant_id WHERE m.deliver_food = ".$deliver_food." and find_in_set(".$condition.", ".$column.") > 0 GROUP BY d.restaurant_id HAVING distance <= 5");
       		
	}
   else
   {
   	  $data = mysqli_query($GLOBALS['conn'],"SELECT d.*,( 3959 * acos( cos(radians($lat) ) * cos( radians(latitude ) ) * cos(radians(longitude ) - radians($long) ) + sin( radians($lat) ) * sin( radians(latitude ) ) ) ) AS distance  FROM `restaurant_details` as d join restaurant_meal_details as m on m.restaurant_id = d.restaurant_id WHERE  find_in_set(".$condition.", ".$column.") > 0 AND m.deliver_food = ".$deliver_food." GROUP BY d.restaurant_id");
   }
   if ($data) {
	  ...
	  if(!empty($meal)){
		  $data = mysqli_query($GLOBALS['conn'],"SELECT d.*,( 3959 * acos( cos(radians($lat) ) * cos(radians(latitude ) ) * cos( radians( longitude ) - radians($long) ) + sin( radians($lat) ) * sin( radians(latitude)))) AS distance FROM `restaurant_details` as d join restaurant_meal_details as m on m.restaurant_id = d.restaurant_id WHERE m.meal =".$meal." AND m.deliver_food = ".$deliver_food." GROUP BY d.restaurant_id Limit $minLimit, $pageSize");	
		}else if(!empty($location)){
		  $data = mysqli_query($GLOBALS['conn'],"SELECT d.*,( 3959 * acos(cos(radians($latt) ) * cos( radians(latitude ) ) * cos(radians(longitude ) - radians($longg) ) + sin( radians($latt) ) * sin( radians(latitude ) ) ) ) AS distance  FROM `restaurant_details` as d join restaurant_meal_details as m on m.restaurant_id = d.restaurant_id WHERE m.deliver_food = ".$deliver_food." and find_in_set(".$condition.", ".$column.") > 0 GROUP BY d.restaurant_id HAVING distance <= 5 Limit $minLimit, $pageSize");	
