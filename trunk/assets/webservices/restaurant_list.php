...
//Receiveing Input in Json and decoding
 ...
 {
     $restaurant_data   = mysqli_query($GLOBALS['conn'],"SELECT *,( 3959 * acos( cos( radians($lat) ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians($long) ) + sin( radians($lat) ) * sin( radians( latitude ) ) ) ) AS distance FROM restaurant_details rd left join restaurant_meal_details rmd on rd.restaurant_id = rmd.restaurant_id WHERE rd.status = 1 group by rd.restaurant_id");
