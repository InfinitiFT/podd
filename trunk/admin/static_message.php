...
try {
      ...
      { 
        ...
        if(mysqli_query($GLOBALS['conn'],"UPDATE `static_page` SET `name`='".$name."',`content`= '".$content."' WHERE id = 1")){
              $_SESSION["successmsg"] = "Policy edited successfully.";
        }
        else{
        ...
        $static_data = mysqli_fetch_assoc(mysqli_query($GLOBALS['conn'],"SELECT * FROM `static_page` WHERE id = 1"));
